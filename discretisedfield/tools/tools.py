...
def topological_charge_density(field, /, method="continuous"):
    ...
    (``nvdims=3``). Two different methods are available and can be selected using
    ``method``:
    ...
    >>> f = df.Field(mesh, nvdims=1, value=12)
    >>> dft.topological_charge_density(f.plane('z'))
    ...
    """
    if field.nvdims != 3:
        msg = f"Cannot compute topological charge density for {field.nvdims=} field."
    ...
    if method == "continuous":
        ...
        q = df.Field(field.mesh, nvdims=1)
...
def topological_charge(field, /, method="continuous", absolute=False):
    ...
    This function computes topological charge for a vector field (``nvdims=3``).
    ...
    """
    if field.nvdims != 3:
        msg = f"Cannot compute topological charge for {field.nvdims=} field."
        raise ValueError(msg)
...
def emergent_magnetic_field(field):
    ...
    """
    if field.nvdims != 3:
        msg = f"Cannot compute emergent magnetic field for {field.nvdims=} field."
        raise ValueError(msg)
...
def neigbouring_cell_angle(field, /, direction, units="rad"):
    ...
    (``nvdims=3``). Angles are computed in degrees if ``units='deg'`` and in
    ...
    """
    if not field.nvdims == 3:
        msg = f"Cannot compute spin angles for a field with {field.nvdims=}."
        raise ValueError(msg)
    ...
    return df.Field(mesh, nvdims=1, value=angles.reshape(*angles.shape, 1))
...
def max_neigbouring_cell_angle(field, /, units="rad"):
    ...
    possible for vector fields (``nvdims=3``). Angles are computed in degrees if
    ...
    return df.Field(field.mesh, nvdims=1, value=max_angles)
