...
class FieldRotator:
    ...
    def __init__(self, field):
        if field.nvdims not in [1, 3]:
            raise ValueError(
                f"Rotations are not supported for fields with{field.nvdims=}."
            )
        if field.mesh.bc != "":
    ...
    def rotate(self, method, /, *args, n=None, **kwargs):
        ...
        if self._orig_field.nvdims == 1:
            rot_field = self._orig_field.array
        elif self._orig_field.nvdims == 3:
            rot_field = self._rotation.apply(
                self._orig_field.array.reshape((-1, self._orig_field.nvdims))
            ).reshape((*self._orig_field.mesh.n, self._orig_field.nvdims))

        # Calculate field at new mesh positions
        new_m = self._map_and_interpolate(new_mesh, rot_field)

        self._rotated_field = df.Field(
            mesh=new_mesh, nvdims=self._orig_field.nvdims, value=new_m
        )
    ...
    def _map_and_interpolate(self, new_mesh, rot_field):
        new_mesh_field = df.Field(mesh=new_mesh, nvdims=3, value=lambda x: x)
        new_mesh_pos = (
        ...
        result = np.ndarray(shape=[*new_mesh_field.mesh.n, self._orig_field.nvdims])
        for i in range(self._orig_field.nvdims):
