...
def test_emergent_magnetic_field():
    ...
    assert dft.emergent_magnetic_field(f).nvdims == 3
...
def test_neigbouring_cell_angle():
    ...
    # Exceptions
    scalar_field = df.Field(mesh, nvdims=1, value=5)
    with pytest.raises(ValueError):
...
def test_count_lange_cell_angle_regions():
    ...
    field = df.Field(mesh, nvdims=3, value={"sub": (0, 0, 1), "default": (0, 0, -1)})
...
def test_demag_field_sphere():
    ...
    f = df.Field(mesh, nvdims=3, value=(0, 0, 1), norm=norm)
...
def test_demag_field_plane():
    ...
    f = df.Field(mesh, nvdims=3, value=(0, 0, 1), norm=1)
    tensor = dft.demag_tensor(mesh)
    assert np.allclose(
        dft.demag_field(f, tensor)((0, 0, 0)),
        (0, 0, -1),
