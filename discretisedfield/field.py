...
class Field(collections.abc.Callable):  # could be avoided by using type hints
    ...
    def _writeovf(self, filename, representation='bin8', extend_scalar=False):
        ...
        header.append(f'Begin: Data {repr_string}')
        footer = [f'End: Data {repr_string}',
                  'End: Segment']

        bheader = ''.join(f'# {line}\n' for line in header).encode('utf-8')
        bfooter = ''.join(f'# {line}\n' for line in footer).encode('utf-8')
        ...
        with open(filename, 'wb') as f:
            ...
            if representation in bin_rep:
                ...
                f.write(struct.pack(bin_rep[representation][1],
                                    bin_rep[representation][0]))
    ...
    def _writeovf_old(self, filename, representation='txt', extend_scalar=False):
